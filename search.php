...
if(!empty($_POST))
{
      ...
      $query ="SELECT * FROM questions WHERE headline like '%" . $aKeyword[0] . "%'";
      
     for($i = 1; $i < count($aKeyword); $i++) {
        if(!empty($aKeyword[$i])) {
            $query .= " OR headline like '%" . $aKeyword[$i] . "%'";
        }
      }
     
     $result = $link->query($query); ?>
     <div align="center">
    <?php echo "You have searched for keywords: " . $_POST['keyword']; ?>
     </div> <?php
...
?>
   ...
   <div class="container-fluid">
      <div class="row">
         ...
         <div class="col-lg-7">
         <?php
                     $queryp="SELECT * FROM questions WHERE headline like '%" . $aKeyword[0] . "%'";
